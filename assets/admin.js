...
( function ( $ ) {
	...
	/* --------------------- Ajustes: comprobar conexión --------------------- */

	function setConnectionStatus( state, message ) {
		$( '#scangeo-connection-status' )
			.attr( 'class', 'scangeo-key-status scangeo-key-' + state )
			.text( message || '' );
	}

	$( document ).on( 'click', '#scangeo-test-connection', function () {
		var $btn = $( this ).prop( 'disabled', true );
		setConnectionStatus( 'checking', 'Comprobando conexión…' );

		$.post( scangeoFixer.ajaxUrl, {
			action: 'scangeo_test_connection',
			nonce: scangeoFixer.nonce
		} ).then(
			function ( res ) {
				$btn.prop( 'disabled', false );
				if ( res && res.success && res.data ) {
					setConnectionStatus( 'ok', res.data.message );
				} else {
					var msg = res && res.data && res.data.message ? res.data.message : 'No se pudo comprobar la conexión.';
					setConnectionStatus( 'error', msg );
				}
			},
			function () {
				$btn.prop( 'disabled', false );
				setConnectionStatus( 'error', 'Error de red al comprobar la conexión.' );
			}
		);
	} );
